 ...
 */
 class Recommendation {
    ...
    getList(mediaID, page=1, perPage=25) {
        if (typeof mediaID !== "number") { throw new Error("mediaID is not a number type!"); }
        if (typeof page !== "number") { throw new Error("page is not a number type!"); }
        if (typeof perPage !== "number") { throw new Error("perPage is not a number type!"); }
     ...
     */
    get(recommendID) {
        if (typeof recommendID !== "number") { throw new Error("recommendID is not a number type!"); }
        ...
        return this.util.send(recommendQuery, { id: recommendID });
    }
}
