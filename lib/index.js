...
class AniList {
	...
	async favouriteStudio(id) {
		if (!id || typeof id !== "number") throw new Error("AniList ID is not provided!");
		...
		return data.ToggleFavourite.studios.nodes.some((e) => e.id === id);
	}
	 ...
	 */
	genres() {
		return this.__util.send("query { GenreCollection }", null).then((data) => data.GenreCollection);
	}
	...
	mediaTags() {
		return this.__util
			...
			.then((data) => data.MediaTagCollection);
	}
}
