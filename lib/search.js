...
class Search {
	...
	anime(term = null, filter = null, page = 1, amount = 5) {
		if (term && typeof term !== "string") throw new Error("Search term is not a string");
		if (typeof page !== "number") throw new Error("Page number is not a number");
		if (typeof amount !== "number") throw new Error("Amount is not a number");

		let query = filterBuilder("anime", filter);

		query += "id status format title { romaji english native userPreferred } } } }";
	...
	activity(activityID = null, filter = null, page = 1, amount = 5) {
		if (activityID && typeof activityID !== "number") throw new Error("activityID is not a number.");
		if (typeof page !== "number") throw new Error("Page number is not a number");
		if (typeof amount !== "number") throw new Error("Amount is not a number");
